...
module.exports = {
    getAbsenByIdUser: async (req, res, next) => {
        const body = req.params.body;

        try {
            const absens = await model.absen.findAndCountAll({
                where: { id_user: body.id }, offset: parseInt(body.page), limit: parseInt(body.size),
                order: [
                    [body.sortField, body.sortOrder],
                ]
            });
            const users = await model.users.findByPk(body.id, { include: ['pegawai'] });
            if (absens.length !== 0) {
                res.json({
                    'status': 'OK',
                    berhasil: true,
                    totalPage: absens.count,
                    limit: body.size,
                    page: body.page,
                    user: users,
                    'data': absens,
    ...
    cekLokasi: (req, res) => {
        ...
        const distance = geolib.getDistance(
            { latitude: -0.916237, longitude: 100.356527 },
